...
# 그러고 나서 우리는 점차적으로 ``torch.nn``, ``torch.optim``, ``Dataset``, 또는
# ``DataLoader`` 로부터 한 번에 하나씩 특징을 추가하면서, 정확히 각 부분이 어떤 일을 하는지 그리고
...
# python 전용 포맷 pickle을 이용하여 저장되어 있습니다.
...
###############################################################################
# 각 이미지는 28 x 28 형태이고, 784 (=28x28) 크기를 가진 하나의 행으로 저장되어 있습니다.
# 한 장을 살펴봅시다; 먼저 우리는 이 이미지를 2d로 재구성해야 합니다.
...
###############################################################################
# PyTorch는 NumPy 배열보다는 ``torch.tensor`` 를 사용하므로, 데이터를 변환해야 합니다.
...
###############################################################################
# 우리의 무작위 모델에 대한 손실을 점검해 봄으로써 역전파 이후에 개선이 있는지 나중에
# 확인할 수 있습니다.
...
#    아래에서 ``set_trace()`` 를 주석 해제하여 사용해 보세요.
#
...
# -----------------------------------
#
# 이제 코드를 리팩토링(refactoring) 하겠습니다. 그로써 이전과 동일하지만,
...
###############################################################################
# 더 이상 ``model`` 함수에서 ``log_softmax`` 를 호출하지 않고 있습니다.
# 손실과 정확도과 이전과 동일한지 확인해 봅시다:
...
#
# .. note:: ``nn.Module`` (대문자 M) 은 PyTorch의 특정 개념이고, 우리는 이 클래스를
#    많이 사용할 것입니다. ``nn.Module`` 를 Python의 코드를 임포트하기 위한 코드 파일인
...
# 그러나 배후에서 PyTorch는 우리의 ``forward`` 메소드를 자동으로 호출합니다.
